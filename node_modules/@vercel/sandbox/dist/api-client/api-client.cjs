...
var APIClient = class extends require_base_client.BaseClient {
	...
	async createSandbox(params) {
		const privateParams = require_types.getPrivateParams(params);
		const endpoint = params.runtime === void 0 ? "/v3/sandboxes" : "/v2/sandboxes";
		return require_base_client.parseOrThrow(require_validators.SandboxAndSessionResponse, await this.request(endpoint, {
			method: "POST",
			body: JSON.stringify({
	...
	async forkSandbox(params) {
		const privateParams = require_types.getPrivateParams(params);
		return require_base_client.parseOrThrow(require_validators.SandboxAndSessionResponse, await this.request(`/v2/sandboxes/${encodeURIComponent(params.sourceSandbox)}/fork`, {
			method: "POST",
