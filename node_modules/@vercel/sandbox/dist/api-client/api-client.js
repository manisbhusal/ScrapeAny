...
var APIClient = class extends BaseClient {
	...
	async createSandbox(params) {
		...
		return parseOrThrow(SandboxAndSessionResponse, await this.request(endpoint, {
			method: "POST",
			body: JSON.stringify({
	...
	async forkSandbox(params) {
		const privateParams = getPrivateParams(params);
		return parseOrThrow(SandboxAndSessionResponse, await this.request(`/v2/sandboxes/${encodeURIComponent(params.sourceSandbox)}/fork`, {
			method: "POST",
			...
			return {
				...
				finished: (async () => {
					while (true) {
						...
						if (chunk.done) throw new StreamError("stream_ended_early", "Stream ended before command finished", params.sessionId);
						if (chunk.value?.command) {
							const { command: command$1 } = CommandFinishedResponse.parse(chunk.value);
							return command$1;
						}
	...
	async openInteractive(params) {
		return parseOrThrow(InteractiveSessionResponse, await this.request(`/v2/sandboxes/sessions/${params.sessionId}/interactive`, {
			method: "POST",
			body: JSON.stringify({}),
			signal: params.signal
		}));
