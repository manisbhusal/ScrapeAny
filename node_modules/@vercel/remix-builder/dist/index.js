...
function getReactRouterDataPaths(path) {
...
function findPrerenderedHtmlFile(path, staticFileKeys) {
  const candidates = path === "index" ? ["index.html"] : [`${path}.html`, `${path}/index.html`];
  return candidates.find((c) => staticFileKeys.has(c));
}
function getPrerenderDocumentRewrite(path, htmlFile) {
  return {
    src: path === "index" ? "^/$" : `^/${path}/?$`,
    dest: `/${htmlFile}`
  };
}
function shouldRegisterSsrForPrerenderedRoute(path) {
  return path === "index";
}
...
var build = async ({
  ...
  try {
    await (0, import_build_utils3.generateProjectManifest)({
      ...
      serviceType: service ? (0, import_build_utils3.getReportedServiceType)(service) : void 0
    });
  } catch (err) {
...
// src/index.ts
var version = 2;
var diagnostics = (0, import_build_utils6.createDiagnostics)("node");
